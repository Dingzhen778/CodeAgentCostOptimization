...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.86\linewidth]{figures/gateway_arch.pdf}
  \caption{本地 Token 计量网关架构。
    网关作为透明代理插入在 mini-swe-agent 与上游模型 API 之间，
    不修改任何请求/响应内容，仅拦截并解析响应中的 \texttt{usage} 字段，
    将每步的输入 token 数、输出 token 数、实验名和 instance\_id 落盘至 JSONL 日志，
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcrccl}
    \toprule
    方法 & 检索数 & 函数裁剪 & LLMLingua & Tok.Budget & 工作流/记忆 \\
    \midrule
    \texttt{baseline\_raw}       & —  & —           & —           & —    & — \\
    \texttt{rag\_topk}           & 4  & —           & —           & —    & — \\
    \texttt{rag\_function}       & 4  & \checkmark  & —           & —    & — \\
    \texttt{llmlingua\_original} & 4  & \checkmark  & \checkmark  & —    & — \\
    \texttt{skill\_abstraction}  & 4  & —           & —           & —    & Workflow \\
    \texttt{skill\_memory\_md}   & 4  & —           & —           & —    & Memory \\
    \texttt{hybrid\_llmlingua}   & 4  & \checkmark  & \checkmark  & 2500 & — \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{3.8cm}llp{4cm}}
    ...
    \texttt{llmlingua\_original} & 上下文压缩   & Search+Read & 语义精选高价值片段 \\
...
\begin{figure}[htbp]
  ...
  \caption{各方法实现的模块依赖关系。
    顶层为七种实验方法；中间为统一的 \texttt{TaskContextBuilder}，
    根据方法类型调用对应组件（BM25 检索器、AST 函数裁剪器、
    Token 预算裁剪器、LLMLingua 排序器或 Skill 注入器）；
    底层共享 \texttt{AgentRunner}—mini-swe-agent—Docker—Token 计量网关的统一执行链路，
    确保各方法在相同基础设施上进行公平比较。}
