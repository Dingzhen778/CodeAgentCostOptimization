...
\texttt{VERIFY}、\texttt{RETRY} 五个阶段，并给出了精确的 词元 度量口径：
...
\textbf{组合路径}（一种方法）：
\begin{itemize}
  \item \texttt{hybrid\_llmlingua}：串联语义重排序、函数级裁剪与 词元 budget 截断
\end{itemize}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{lXrr}
    ...
    \texttt{llmlingua\_original}& 词元 削减最多（$-$5.74\%），但解决率下降  & $-$1.0 pp & $-$2.9\%  \\
...
失败实例占比约 66\%，其 词元 消耗约占总成本的 83\%。
这意味着降低 CPR 的最有效杠杆不是上下文压缩，而是\textbf{提升解决率或引入有效早停}。

\textbf{发现二：代码阅读（understand）阶段是 词元 消耗主体（约 50\%），
...
两者均同时提升解决率并降低平均 词元 消耗，达到 Pareto 改善。
从机制层面，降低 CPR 的三个有效杠杆依次为：
（1）提升解决率（将失败变为成功）；
（2）减少冗余搜索与阅读（改善搜索效率）；
（3）稳定工作流（减少规划空转）。
纯粹的 词元 压缩在不改善解决率的前提下，CPR 改善空间有限。
...
然而，实验结果表明失败轨迹的平均 词元 消耗是成功轨迹的 2--3 倍，
