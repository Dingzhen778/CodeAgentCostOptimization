...
\textbf{语义相关性重排序}，以替代 BM25 的词频统计。llmlingua\_original的原理如图~\ref{fig:overviewLLMLingua}所示。
\begin{figure}[htbp]
  \centering
...
%% ─────────────────────────────────────────────────────────────────────────────
\section{方法五：skill\_abstraction（工作流技能构建）}
...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{4.2cm} l X}
\toprule
参数 & 值 & 说明 \\
\midrule
...
\texttt{rag\_topk}           & 上下文压缩 & 减少全仓搜索 \\
\texttt{rag\_function}       & 上下文压缩 & 减少文件内冗余 \\
\texttt{llmlingua\_original} & 上下文压缩 & 语义精选高价值片段 \\
\texttt{skill\_abstraction}  & 工作流压缩 & 减少规划空转 \\
\texttt{skill\_memory\_md}   & 工作流压缩 & 复用跨任务经验 \\
\texttt{hybrid\_llmlingua}   & 两路结合   & 同时压文件数和长度 \\
\bottomrule
\end{tabularx}
